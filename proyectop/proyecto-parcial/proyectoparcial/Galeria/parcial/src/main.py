...
def main(page: ft.Page):
    ...
    def mostrar_artista():
        ...
        contenedor.content=ft.Column([
            ft.Image(cantante["imagen"],widht=300,height=400,fit=ft.ImageFit.CONTAIN),
            ft.Text(cantante["titulo"],size=20,weight=ft.FontWeight.BOLD),
            ft.Text(f"descripcion: {cantante['descripcion']}",size=16,italic=True)
        ],alignment=ft.Mai)
        
        def mostrar_albums():
            albums = album[indice_actual[0]]
            contenedor.content=ft.Column([
            ft.Image(album ["imagen"],widht=300,height=400,fit=ft.ImageFit.CONTAIN),
    ...
    if indice_actual[0]==len(album)-1:
        ...
        boton_siguiente.text="Siguiente album"
        page.update()
        
    def siguiente_click(e):
        indice_actual[0] = (indice_actual[0] + 1) % len(album)
        mostrar_artista()
